...
# --- DISPLAY IMAGES ---
def show_image_triplet(input_img, pred_img, target_img, idx, iou, dice):
    fig, axes = plt.subplots(1, 3, figsize=(12, 4))
    titles = ['Input (Noisy Road)', f'Predicted Skeleton\nIoU={iou:.3f}, Dice={dice:.3f}', 'Ground Truth']
...
def evaluate(model_path=MODEL_SAVE_PATH, num_samples=10):
    ...
    with torch.no_grad():
        for idx, (image, target) in enumerate(loader):
            ...
            show_image_triplet(image.cpu().numpy()[0, 0], pred, target_np, idx, iou, dice)
