...
CONFIGS = {
    "eaton-town": {
        "place": "Eaton, Ohio, USA",
        "image_size": (256, 256),
        "n_samples": 500,
        "network_type": "drive",  
        'data_dir': 'eaton_data/thinning'
    },
}
...
def generate_samples(gdf_edges, gdf_nodes, n_samples=500, out_dir="dataset", image_size=(256, 256)):
    ...
    for i, (_, node) in enumerate(tqdm(selected_nodes.iterrows(), total=n_samples), start=10000):
        pt = node.geometry
...
if __name__ == "__main__":
  ...
  generate_samples(edges, nodes, n_samples=config['n_samples'], out_dir=config.setdefault('data_dir', './eaton_data/thinning'))
