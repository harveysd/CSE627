...
def pick_random_intersections(nodes, n=500):
    ...
    if n > available_nodes:
        print(f" Requested {n} samples, but only {available_nodes} nodes available. Sampling {available_nodes} instead.")
        n = available_nodes
    return nodes.sample(n), n
...
def generate_samples(gdf_edges, gdf_nodes, n_samples=500, out_dir="dataset", image_size=(256, 256)):
    ...
    selected_nodes, n_samples = pick_random_intersections(gdf_nodes, n=n_samples)
